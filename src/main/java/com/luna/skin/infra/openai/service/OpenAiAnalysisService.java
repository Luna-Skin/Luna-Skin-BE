...
public class OpenAiAnalysisService {
  ...
  private String buildPrompt(String phaseType) {
    return """
                당신은 피부과 전문의입니다. 20대 여성 사용자의 얼굴 피부 사진을 분석해주세요.
                현재 생리 주기 단계는 %s입니다.

                반드시 아래 JSON 형식으로만 응답하세요. 다른 텍스트는 포함하지 마세요.
                점수는 0, 20, 40, 60, 80, 100 중 하나이며, 모든 지표는 점수가 높을수록 좋은 상태를 의미합니다.
                사진의 조명이나 화질로 인한 단순한 밝기·반사광을 피부의 유분이나 칙칙함으로 오인하지 말고,
                실제 피부 영역의 특징을 중심으로 판단하세요.

                각 항목 채점 기준 (측정값을 먼저 산출한 뒤 아래 기준으로 점수를 결정):
                - trouble(트러블): 감지된 트러블/여드름 개수 기준. 0개=100점, 1개=80점, 2~3개=60점, 4~6개=40점, 7~10개=20점, 11개 이상=0점.
                  개수가 적을수록 좋은 상태.
                - sebum(유분): 피지량 측정값 기준. 0~15=100점, 16~30=80점, 31~45=60점, 46~65=40점, 66~80=20점, 81~100=0점.
                  측정값이 낮을수록 좋은 상태.
                - dullness(칙칙함): 피부 톤 균일도 측정값 기준. 0~15=100점, 16~30=80점, 31~45=60점, 46~65=40점, 66~80=20점, 81~100=0점.
                  측정값이 낮을수록 맑은 상태로 판단해 높은 점수 부여.
                - moisture(수분): 피부 수분량 측정값 기준. 78~100=100점, 68~77=80점, 58~67=60점, 43~57=40점, 28~42=20점, 0~27=0점.
                  측정값이 높을수록 좋은 상태.
                - elasticity(탄력): 피부 탄력 측정값 기준. 85~100=100점, 75~84=80점, 65~74=60점, 50~64=40점, 35~49=20점, 0~34=0점.
                  측정값이 높을수록 좋은 상태.
